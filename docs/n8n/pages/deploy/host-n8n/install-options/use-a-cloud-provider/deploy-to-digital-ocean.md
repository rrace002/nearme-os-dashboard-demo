...
The AI Assistant needs a sandbox to run code in. You can add the same sandbox stack used in the [Docker Compose guide](/deploy/host-n8n/install-options/install-using-docker-compose.md) to this setup. A few things are worth knowing before you start:
...
5. Restart everything so the new services pick up the changes:
   ...
   sudo docker compose up -d
   ```
6. Add your model API key. See [Set up AI Assistant](/deploy/host-n8n/configure-n8n/set-up-ai-assistant.md) for the full reference, including how to pick a model provider. Web search works out of the box via the bundled SearXNG service above; add a Brave Search key instead if you'd rather use that.
