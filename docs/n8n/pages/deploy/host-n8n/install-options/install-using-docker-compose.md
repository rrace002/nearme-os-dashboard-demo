> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/deploy/host-n8n/install-options/install-using-docker-compose.md).
...
If you just want n8n (and the AI Assistant) running quickly without writing any files yourself, use the [one-line setup](/deploy/host-n8n/install-options/one-line-setup.md) instead. It sets up everything below automatically.
...
This bundles n8n's own sandbox (`n8n-sandbox`), which is a good fit for local development and testing. For a production instance, n8n currently recommends Daytona as the sandbox provider instead. See [Set up the AI Assistant](/deploy/host-n8n/configure-n8n/set-up-ai-assistant.md) for how to configure a Daytona sandbox.
...
Full setup steps, including the supported model providers, are in [Set up the AI Assistant](/deploy/host-n8n/configure-n8n/set-up-ai-assistant.md).
