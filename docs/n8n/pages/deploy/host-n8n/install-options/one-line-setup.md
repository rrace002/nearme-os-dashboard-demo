> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/deploy/host-n8n/install-options/one-line-setup.md).
...
* **Already self-hosting with your own Docker Compose file?** You don't need to switch to this script, but feel free to take inspiration from [the Docker Compose setup process](/deploy/host-n8n/install-options/install-using-docker-compose.md).
...
If you're setting n8n up for a team or a production environment, consider a more robust database like Postgres rather than the built-in default. See [Install using Docker Compose](/deploy/host-n8n/install-options/install-using-docker-compose.md) for that setup.

The same goes for the sandbox: this setup uses n8n's own bundled sandbox, which is a good fit for trying things out, but for production, n8n currently recommends Daytona instead. See [Set up the AI Assistant](/deploy/host-n8n/configure-n8n/set-up-ai-assistant.md) for how to configure it.
...
Full setup steps, including the supported providers, are in [Set up the AI Assistant](/deploy/host-n8n/configure-n8n/set-up-ai-assistant.md).
