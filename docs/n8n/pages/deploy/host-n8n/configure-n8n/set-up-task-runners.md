...
Task runners are the only isolation layer between user-provided code and n8n. Without them, or with [internal mode](#internal-mode), anyone who can edit a workflow could potentially read your database, [encryption key](/deploy/host-n8n/configure-n8n/basic-configuration/configuration-examples/set-a-custom-encryption-key.md), stored credentials, and environment variables.

In production, and on any instance holding sensitive data, use [external mode](#external-mode) plus the measures in [Hardening task runners](/deploy/host-n8n/configure-n8n/security/harden-task-runners.md). Skipping external mode to save hosting costs is only a reasonable tradeoff on isolated instances that hold nothing but trusted or mock data.
{% endhint %}
