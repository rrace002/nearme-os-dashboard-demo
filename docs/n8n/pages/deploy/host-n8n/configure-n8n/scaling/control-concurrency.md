> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/deploy/host-n8n/configure-n8n/scaling/control-concurrency.md).
...
This document is for self-hosted concurrency control. Read [Cloud concurrency](/deploy/use-n8n-cloud/understand-concurrency.md) to learn how concurrency works with n8n Cloud accounts.
{% endhint %}
...
In queue mode, you can control how many jobs a worker may run concurrently using the [`--concurrency` flag](/deploy/host-n8n/configure-n8n/scaling/enable-queue-mode.md#configure-worker-concurrency).
...
Evaluation test runs use a separate concurrency limit from production executions. By default, the limit follows the instance's license tier (Community/Pro 1, Business 3, Enterprise 5). Override it with [`N8N_CONCURRENCY_EVALUATION_LIMIT`](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables/executions.md). Refer to [Metric-based evaluations](/build/integrate-ai/test-and-improve-ai-workflows/use-metrics-to-measure-quality.md#run-test-cases-in-parallel) for how the slider behaves in the UI.
