...
{% hint style="info" %}
On n8n Cloud, supported AI models and services don't need a credential at all: select **Use Gateway credits** on the node to run it with [Gateway credits](/build/understand-workflows/use-gateway-credits.md) instead of your own API key.
{% endhint %}
...
End-user credentials let a workflow run with the credentials of the person who triggers it, and keep each user's data private to them. The choice between **Fixed credential** and **End-user credential** types appears for OAuth credentials on n8n Cloud Enterprise and self-hosted Enterprise. To learn how to set them up, see [End-user credentials](/administer/manage-credentials/end-user-credentials.md).
