> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/build/integrate-ai/mcp-servers.md).
...
You can also connect MCP servers in the registry to the [AI Assistant](/build/ways-of-building-workflows/ai-assistant.md).

## Add a registry server to an agent

1. Open the tools panel on an [AI Agent](/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent.md) node in the canvas (by clicking on "Tool +"), or in [Build and manage agents](/build/build-and-manage-agents.md#add-tools) by clicking on "Add Tool".
...
n8n creates an [MCP credential](/integrations/builtin/credentials/mcp.md) automatically when you sign in from the panel.
...
If the server you want isn't in the registry yet, use the [MCP Client Tool](/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.toolmcp.md) node to connect to any MCP server manually with its connection URL and credentials.
...
| **The agent supplies** | Only the fields you [leave open to the model](/build/integrate-ai/ai-examples/use-ai-for-parameters.md), you choose | Which tool call to make (out of the allowed tools) and all of its arguments |
