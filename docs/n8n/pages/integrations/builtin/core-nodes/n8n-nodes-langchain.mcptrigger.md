> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/integrations/builtin/core-nodes/n8n-nodes-langchain.mcptrigger.md).
...
Unlike conventional [trigger nodes](#user-content-fn-1)[^1], which respond to events and pass their output to the next [connected node](/build/understand-workflows/workflow-components/connect-nodes-together.md), the MCP Server Trigger node only connects to and executes [tool](/build/integrate-ai/understand-ai-components/how-tools-work.md) nodes. Clients can list the available tools and call individual tools to perform work.
...
The MCP Server Trigger node relies on Server-Sent Events (SSE) or streamable HTTP, which require the same server instance to handle persistent connections. This can cause problems when running n8n in [queue mode](/deploy/host-n8n/configure-n8n/scaling/enable-queue-mode.md) depending on your [webhook processor](/deploy/host-n8n/configure-n8n/scaling/enable-queue-mode.md#webhook-processors) configuration:
