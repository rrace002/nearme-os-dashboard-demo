> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/integrations/builtin/core-nodes/n8n-nodes-base.code/common-issues.md).

# Common issues

Here are some common errors and issues with the [Code node](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) and steps to resolve or troubleshoot them.
...
* Read the [data structure](/build/work-with-data/understand-n8ns-data-structure.md) to understand the data you receive in the Code node and the requirements for outputting data from the node.
* Understand how data items work and how to connect data items from previous nodes with [item linking](/build/work-with-data/reference-data/link-data-items.md).
...
To resolve this, ensure that your Code node returns the [expected data structure](/build/work-with-data/understand-n8ns-data-structure.md):
...
n8n doesn't support importing modules in the [Cloud](/deploy/use-n8n-cloud.md) version.
{% endhint %}

If you're [self-hosting](/deploy/host-n8n.md) n8n, follow these steps:

* Install the module into your n8n environment.
  * If you are running n8n with [npm](/deploy/host-n8n/install-options/install-with-npm.md), install the module in the same environment as n8n.
  * If you are running n8n with [Docker](/deploy/host-n8n/install-options/install-with-docker.md), you need to extend the official n8n image with a [custom image](https://docs.docker.com/build/building/base-images/) that includes your module.
* Set the `NODE_FUNCTION_ALLOW_BUILTIN` and `NODE_FUNCTION_ALLOW_EXTERNAL` [environment variables](/deploy/host-n8n/configure-n8n/basic-configuration/configuration-examples/enable-modules-in-code-node.md) to allow importing modules.
...
To set, update, and retrieve data directly to a workflow, use the [static data](/build/code-in-n8n/cookbook/built-in-methods-and-variables-examples/getworkflowstaticdata.md) functions within your code. You can manage data either globally or tied to specific nodes.

{% hint style="info" %}
**Use Remove Duplicates when possible**

If you're interested in using variables to avoid processing the same data items more than once, consider using the [Remove Duplicates node](/integrations/builtin/core-nodes/n8n-nodes-base.removeduplicates.md) instead. The Remove Duplicates node can save information across executions to avoid processing the same items multiple times.
{% endhint %}
...
If you need to make authenticated API calls, use the [HTTP Request node](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) which provides credential support.
