...
If you select **Predefined Credential Type**, a **Credential Type** parameter appears. Select the app or service you want to use, then select an existing credential for it or create a new one. Refer to [Custom API operations](/integrations/builtin/custom-api-actions-for-existing-nodes.md) for more information.

If you select any other authentication method except **None**, a **Credential for** parameter appears for you to select an existing or create a new authentication credential for that authentication type.
