...
These options are available from n8n 2.36, on Schedule Trigger nodes added from that version on. They take effect only when the instance runs the [durable scheduler](/deploy/host-n8n/configure-n8n/durable-scheduler.md); the default in-memory scheduler never runs missed executions.
{% endhint %}
