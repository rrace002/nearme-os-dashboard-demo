...
![The Instance-level MCP page before you enable MCP access, showing the Enable MCP access button](/files/PUJNCZZylyS5aQcB4Bxe)
...
![The Instance-level MCP page after you enable MCP access, showing Connection details, Access, and Connected clients](/files/rjSaEabeFIQR60TS1PSc)
...
Per-client connection setup steps are available from n8n 2.33.0. Earlier versions don't show these steps. Use the manual instructions in [MCP client connection examples](/connect/connect-to-n8n-mcp-server/mcp-client-examples.md) to configure your client instead.
{% endhint %}
...
5. Follow the steps shown for your client type:
   ...
   * **CLI clients**: run the install command shown, or add the manual configuration snippet to the client's configuration file instead. Either way, finish with the **Authenticate** step to complete the OAuth sign-in (see the [Claude Code](/connect/connect-to-n8n-mcp-server/mcp-client-examples.md#connecting-claude-code-to-n8n-mcp-server) and [Codex](/connect/connect-to-n8n-mcp-server/mcp-client-examples.md#connecting-codex-cli-to-n8n-mcp-server) examples for the exact commands).
...
![Options menu with Manage MCP access expanded, showing Enable MCP access and Disable MCP access](/files/htt9zqEDAMaXMmyTnIFj)
...
2. Option 2: From the workflow editor
   ...
   ![Workflow's main menu open, with Edit description highlighted](/files/HTxhDyB4YH17wLvwWa8Y)
...
Agents are available from n8n 2.34.0 and are a separate feature from workflows. See [Build and manage agents](/build/build-and-manage-agents.md) for details. This section only applies if you've enabled agents on your instance.
{% endhint %}
...
The n8n MCP server exposes tools for workflow management, workflow building, agent management, and data tables. For a complete list of available tools and their parameters, refer to the [MCP server tools reference](/connect/connect-to-n8n-mcp-server/mcp-server-tools-reference.md).
...
For exact commands and configuration to connect specific clients, including Lovable, Claude Desktop, Claude Code, Codex, Gemini CLI, Cursor, VS Code, Windsurf, and Google ADK agents, see [MCP client connection examples](/connect/connect-to-n8n-mcp-server/mcp-client-examples.md).
