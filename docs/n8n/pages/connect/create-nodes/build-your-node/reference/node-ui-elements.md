...
![String field example: a Name input with Fixed/Expression toggle and the value n8n typed in](/files/9D9KQSVCIHVNTuP9BtAS)
...
![Password field example: a Password input showing masked dots instead of the typed value](/files/7D0rQAozFsTjgLzjVHqp)
...
![Multi-row string field example: a Description textarea spanning several lines of text](/files/9IUQq64Y1lIzDmhobLb1)
...
![Decimal number field example: an Amount input showing the value 10.00](/files/J4aJSCWEEjs1RzsXzSRY)
...
![Collection field example: a Filters group with an optional Type dropdown open, listing Automated, Past, and Upcoming](/files/CoUZBYzXfMhNdr19jmIi)
...
![DateTime field example: a date input opening a calendar picker to choose a date and time](/files/nKl7kDBWv6rH5GWagXq9)
...
![Boolean field example: a Wait for Image toggle switched on](/files/QFSeHVfNLk9ssot7DtKk)
...
![Color field example: a Background Color input with its picker swatch expanded](/files/omthgbCp46SjONZBhMg1)
...
![Options field example: a Resource dropdown open showing single-select choices Image and Template](/files/Vwas9OySKVnEeKzE9DlE)
...
![Multi-options field example: an Events field with Plan Created and Plan Deleted both selected as removable tags](/files/k2WKZaZ5FjRNJPEB1RSk)
...
![Filter field example: a Conditions builder with two rules joined by OR, using ends with and contains operators](/files/nE6GFhfYOe8I2xREKH5n)
...
![Fixed collection example: a Metadata group with two repeated Name/Value field pairs and an Add Metadata button](/files/jH1eqKyViQ8r8WQBL48Y)

## Resource locator <a href="#resource-locator" id="resource-locator"></a>

![Resource locator field example: a Card field with its mode dropdown open, offering By ID, By URL, and From list](/files/s7StewkxpU2yHlnr3ZOW)
...
![JSON field example: a Content (JSON) code editor with syntax-highlighted keys and values](/files/aAkdKIMdv2gvX0cxPdKu)
...
![Notice field example: a pale yellow info box displaying placeholder text](/files/5zk4jEbpyTkdeu5xWuOr)
